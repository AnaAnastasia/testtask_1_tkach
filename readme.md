...
## Что сделано
Создан сервис для туроператора, позволяющий автоматически проверять отели по набору бизнес-правил, заданных для каждого агентства. 
При открытии страницы с отелем система прогоняет его данные через все активные правила и выводит менеджеру предупреждения.

### Архитектура
- SOLID — каждый класс отвечает за одну зону ответственности, классы открыты для расширения новыми данными, но закрыты для правки существующей бизнес-логики, все зависимости (PDO) — через абстракцию DbConnection
- PSR-4 — автозагрузка через Composer
- MVC-подход — контроллеры → сервисы/репозитории → view.
- Post/Redirect/Get для форм (редирект после POST)

### Дальнейшее развитие
- Добавить роутинг или перейти на фреймворк
- Вынести интерфейсы
- Внедрить валидацию для форм
- Добавить красоту

### UPD
Бросила себе вызов - попробовать сделать тестовое задание без фреймворка.
По факту оказалось сложнее, чем писать на Laravel (например, работать с моделями намного проще), но спасибо за такую возможность потренироваться.